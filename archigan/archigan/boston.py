...
def parse_GIS_bostonbuildings_2016(path):
    ...
    #parcels = os.path.join(path, 'Parcels_2023/parcels_23.shp')
    
    parcels = os.path.join(path, 'Parcels_2016/Parcels_2016_Data_Full.shp')
    print(f'Reading parcel data: {parcels}')
    parcels = gpd.read_file(parcels)
    
    #buildings = os.path.join(path, 'structures_poly/structures_poly.shp')
    buildings = os.path.join(path, 'boston_buildings/Boston_Buildings_with_Roof_Breaks.shp')
    ...
    buildings.crs = CRS.from_epsg(epsg)
    ...
    for j, row in tqdm(buildings.iterrows(), desc='Preparing parcel/footprint data'):
        if row.OBJECTID in pidlookup:
            parcel = _parse_polygon(parcels.iloc[pidlookup[row.GIS_ID]].geometry)
